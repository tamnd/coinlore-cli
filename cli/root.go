// Package cli assembles the coinlore command tree from the coinlore
...
// NewApp assembles the kit application from the coinlore domain. The
...
//
// To add a command, declare it in coinlore/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := coinlore.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(coinlore.Domain{}).Register(app)
