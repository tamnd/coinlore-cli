...
describes what users get. Pick a format with `-o`, or let coinlore choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
coinlore <command> -o table   # aligned columns for reading
coinlore <command> -o jsonl   # one JSON object per line, for piping
coinlore <command> -o json    # a single JSON array
coinlore <command> -o csv     # spreadsheet friendly
coinlore <command> -o tsv     # tab-separated
coinlore <command> -o url     # just the URL column
coinlore <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
coinlore <command> --fields id,title,url
```
...
```bash
coinlore <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
coinlore <command>            # a table, because this is a terminal
coinlore <command> | wc -l    # JSONL, because this is a pipe
```
